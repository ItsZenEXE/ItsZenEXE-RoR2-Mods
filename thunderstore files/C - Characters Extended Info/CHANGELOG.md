# Changelog

## 4.3.4 (Current Patch)

- Fixed a bug causing this to apply to every language, not just English.

## 4.3.3

- Corrected a TON of character info that was either outdated or just incorrect due to looking off the wiki early on and never correcting it. Sorry about that!

## 4.3.2

- Internally updated a lot of styling errors
- Fixed many typos.
- Adjusted wording on a few things.

## 4.3.1

- Updated github links
- Updated README slightly
- Updated Dependencies

## 4.3.0

- Updated for current patch (1.4.1)
- Reorganized internally for my own sake

## 4.2.4

- Updated dependencies across the modpack

## 4.2.3

- Added new characters info

## 4.2.2

- Updated modpack in general for parity reasons, but functionally the same package under the hood for this mod in particular

## 4.2.1

- Updated Spare Drone Parts

## 4.2.0

- Updated for DLC3
  - Added new lines for some of the new content, subject to adjustments/changes down the line.

## 4.1.9

- Updated dependencies & README

## 4.1.8

- Updated dependencies

## 4.1.4 - 4.1.7

- Updated other mods, updating the version number for parity with other parts of the modpack.

## 4.1.3

- Fixed the mods downgrading certain dependencies
- Adjusted the mods to only affect English files.

## 4.1.2

- Didnt happen!

## 4.1.1

- Updated the readme again because I was dumb with a link whoops

## 4.1.0

- Updated the icons and readmes for all involved mods.

## 4.0.6

- Adjusted the description for 57 Leaf Clover and Purity to be better on the eyes.
- Added custom descriptions for a few items that were missed the first time around.

## 4.0.5

- This never happened. Dont.

## 4.0.4

- Removed the giant readme thingy from the main menu, because its overreaching for a mod and was meant mainly for my personal testing with friends.

## 4.0.2

- Removed the pings adjustments, as well as the info expansions in the character select screen in the logbook.
  - This was done in order to package them into separate mods altogether to give players choice on the parts of the modpack they want to enable and disable easily.
  - You can find the full modpack now here: <https://thunderstore.io/package/ItsZenEXE/ItsZenEXEs_ROR2_Modpack/>
